...
namespace IMS.Infrastructure.Repository
{
    public class Account(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager) : IAccount
    {
        ...
        {
            if (string.IsNullOrEmpty(model.Policy)) return new ServiceResponse(false, "No Policies Specified");
            Claim[] userClaims = [];
        ...
        public async Task<ServiceResponse> LoginAsync(LoginUserRequestDTO model)
        {
            ...
            if (!verifyPassword.Succeeded) return new ServiceResponse(false, "Incorrect credentisls provided!");
            var result = await signInManager.PasswordSignInAsync(user, model.Password, false, false);
            if (!result.Succeeded) // ServiceResponse(false, "Unkown error occured while login in" // ServiceResponse(true, null)
                return new ServiceResponse(false, "Unkown error occured while login in");
            else
        ...
        public async Task<IEnumerable<GetUserWithClaimsResponseDTO>> GetUserWithClaimsAsync()
        {
            var UserList = new List<GetUserWithClaimsResponseDTO>();
